...
class AbstractCollect(HardwareObject, object):
    ...
    def _pre_collect(self, owner, cp):
        ...
        cp.dangerously_set("synchrotronMode", "4/4")
    ...
    def _store_data_collection_in_lims(self, cp):
        ...
        if HWR.beamline.lims and not cp["in_interleave"]:
            try:
                ret = HWR.beamline.lims.store_data_collection(
                    cp, self.bl_config
                )
                if len(ret) == 2:
                    (self.collection_id, self.detector_id) = ret
                elif len(ret) == 3:
                    print(f"store_data_collection returned {ret}, please check")
                    self.collection_id, self.detector_id = -1, -1
                logging.getLogger("HWR").info(f"collection_id {self.collection_id}, detector_id {self.detector_id}")
            except:
                print(f"storing data collection unsuccessful: it returned {ret}")
                traceback.print_exc()
            ...
            try:
                ...
                cp.dangerously_set("collection_id", self.collection_id)
                
                if self.detector_id:
                    cp.dangerously_set("detector_id", self.detector_id)
    ...
    def _store_sample_info_in_lims(self, cp):
        ...
        Descript. :
        """
        log.info("Collect: Storing sample info in LIMS %s %s " % (type(cp), cp))
        #cpok = cp.to_mutable()
        #if 'take_video' in cp:
            #del cpok['take_video']
            #del cpok['take_snapshots']
            #del cpok['fileinfo']
            #del cpok['in_queue']
            #del cpok['in_interleave']
        #cpok = DataObject(cpok)
                
        if HWR.beamline.lims and not cp["in_interleave"]:
            #HWR.beamline.lims.update_bl_sample(cpok)
    ...
    def _take_crystal_snapshots(self, cp):
        ...
        if number_of_snapshots > 0 and not cp["in_interleave"]:
            ...
            for snapshot_index in range(number_of_snapshots):
                ...
                if hasattr(cp, "dangerously_set"):
                    cp.dangerously_set(
                        "xtalSnapshotFullPath%i" % (snapshot_index + 1), snapshot_filename
                    )
                else:
                    cp["xtalSnapshotFullPath%i" % (snapshot_index + 1)] = snapshot_filename
        ...
        if not HWR.beamline.diffractometer.in_plate_mode() and cp.get("take_video"):
            ...
            if hasattr(cp, "dangerously_set"):
                ...
                cp["xtalSnapshotFullPath2"] = animation_filename
    ...
    def _update_data_collection_in_lims(self, cp):
        ...
        if HWR.beamline.lims and not cp["in_interleave"]:
            ...
            try:
                dd = HWR.beamline.detector.distance.get_value()
                cp.dangerously_set(
                    "detectorDistance", dd
                )
            except:
                log.info("could not read detector distance, please check")
                traceback.print_exc()
    ...
    def stop_collect(self):
        ...
        """
        logging.info('stop_collect called')
