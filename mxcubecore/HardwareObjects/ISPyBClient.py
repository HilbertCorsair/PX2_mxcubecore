...
def trace(fun):
    def _trace(*args):
        ...
        logging.getLogger("ispyb_client").info(log_msg)
        result = fun(*args)
        ...
        logging.getLogger("ispyb_client").info(result_msg)
        return result
...
class ISPyBClient(HardwareObject, speech):
    """
    ...
    def __init__(self, name):
        ...
        speech.__init__(self, port=5555, service="ispyb", verbose=False)
    ...
    def update_data_collection(self, mx_collection, wait=False):
        ...
        :returns: None
        """
        logging.debug('mx_collection %s' % mx_collection)
        if self._disabled:
            return
    ...
    def update_bl_sample(self, bl_sample):
        ...
        """
        logging.debug('update_bl_sample %s' % str(bl_sample))
        if self._disabled:
            return {}
...
class ISPyBValueFactory:
    ...
    def from_data_collect_parameters(ws_client, mx_collect_dict):
        ...
        """
        logging.debug('mx_collect_dict %s' % mx_collect_dict)
        ...
        try:
            data_collection.synchrotronMode = "4/4" # mx_collect_dict["synchrotronMode"]
        ...
        try:
            current_transmission = tr.get_transmission()
            logging.debug(f'transmission from mx_collect_dict {mx_collect_dict["transmission"]}')
            logging.debug(f'transmission from transmission object instance {current_transmission}')
            data_collection.transmission = current_transmission  # mx_collect_dict["transmission"]
        except KeyError:
            pass
