...
class PX2Diffractometer(GenericDiffractometer):
    ...
    def __init__(self, *args):
        ...
        self.oa = optical_alignment.optical_alignment(
            directory="%s/manual_optical_alignment" % os.getenv("HOME"),
            name_pattern="autocenter_%s" % os.getuid(),
        )
    ...
    def save_click(
        ...
        if name_pattern is None:
            name_pattern = "%s_%s_double_click" % (
                os.getuid(),
                time.asctime(time.localtime(timestamp)).replace(" ", "_"),
            )
    ...
    def manual_centring(
        ...
        # "autocenter_%s_element_%s_%s" % (os.getuid(), element, time.asctime().replace(' ', '_'))
        element = self.get_element()
        name_pattern = "manualcenter_%s_element_%s_%s" % (
            os.getuid(),
            element,
        ...
        self.oa.history_start = time.time()
    ...
    def excenter(
        ...
        # )
        execute_line = (
            "/usr/local/conda/envs/murko_3.11/bin/python /usr/local/experimental_methods/shape_from_diffraction_tomography.py -d %s -n %s -D &"
            % (
                directory,
                name_pattern,
            )
