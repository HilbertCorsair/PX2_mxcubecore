...
from mxcubecore.HardwareObjects.mockup.EnergyScanMockup import EnergyScanMockup
try:
...
class PX2EnergyScan(EnergyScanMockup):
    ...
    def startEnergyScan(
        ...
        self.log.info(
            "PX2EnergyScan: energy_scan parameters:\n\tname_pattern: %s\
                                                        \n\tdirectory: %s\
                                                        \n\telement: %s\
                                                        \n\tedge: %s"
            % (name_pattern, directory, element, edge)
        )

        self.experiment = energy_scan(
            name_pattern,
            directory,
            element,
            edge,
        ...
        self.scan_info["startEnergy"] = self.experiment.get_start_energy() / 1.0e3
        self.scan_info["endEnergy"] = self.experiment.get_end_energy() / 1.0e3
    ...
    def scanCommandFailed(self, *args):
        ...
        self.scan_info["startEnergy"] = self.experiment.start_energy / 1.0e3
        self.scan_info["endEnergy"] = self.experiment.end_energy / 1.0e3
    ...
    def get_comment(self):
        comment = ""
        try:
            comment += self.experiment.description
        except:
            pass
        return comment

    def doChooch(
        self, element, edge, directory, archive_directory, prefix, rm_offset=0.03
    ):
    ...
    def get_scan_data(self):
        ...
        """
        self.scan_data = [
            tuple(self.experiment.energies),
            tuple(self.experiment.counts),
        ]
