...
class DataCollectionQueueEntry(BaseQueueEntry):
    ...
    def collect_dc(self, dc, list_item):
        log = logging.getLogger("user_level_log")
        logging.info('dc in collect_dc %s ' % dc)
        if HWR.beamline.collect:
            ...
            try:
                if dc.experiment_type is EXPERIMENT_TYPE.HELICAL:
                    ...
                    HWR.beamline.collect.set_centred_position(start_cpos.as_dict())
                    ...
                    HWR.beamline.collect.set_centred_position(cpos.as_dict())
                else:
                    HWR.beamline.collect.set_helical(False)
                    HWR.beamline.collect.set_mesh(False)
                    HWR.beamline.collect.set_centred_position(cpos.as_dict())
    ...
    def collect_finished(self, owner, state, message, *args):
        ...
        try:
            ...
            if dc.experiment_type is EXPERIMENT_TYPE.MESH:
                dc.grid.set_overlay_pixmap(HWR.beamline.collect.experiment.get_overlay_image_name())
        except:
            logging.getLogger('HWR').exception(traceback.format_exc())
...
class XRFSpectrumQueueEntry(BaseQueueEntry):
    ...
    def xrf_spectrum_finished(self, mcaData, mcaCalib, mcaConfig):
        logging.getLogger().info('xrf_spectrum_finished mcaData %s mcaCalib %s mcaConfig %s' % (str(mcaData[:20]), str(mcaCalib), str(mcaConfig)))
        xrf_spectrum = self.get_data_model()
...
def mount_sample(view, data_model, centring_done_cb, async_result, separator="_"):
    ...
    if hasattr(sample_mount_device, "__TYPE__"):
        if sample_mount_device.__TYPE__ in ["Marvin", "CATS"]:
            element = "%d%s%02d" % (loc[0], separator, loc[1])
            log.info('mount_sample %s' % element)
    ...
    has_loaded_sample = sample_mount_device.has_loaded_sample()
    ...
    if not has_loaded_sample:
        ...
        if dm is not None:
            ...
            try:
                ...
                log.info('centring_method %s ' % centring_method)
                if centring_method == CENTRING_METHOD.MANUAL:
                    ...
                    dm.start_centring_method(dm.C3D_MODE)
                elif centring_method in [CENTRING_METHOD.DOUBLE_CLICK, CENTRING_METHOD.REFRACTIVE]:
                    start_align = time.time()
                    #centrage automatique lors du chargement
                    #dm.align_from_single_image(n_views=4)
                    name_pattern = "autocenter_%s_element_%s_%s" % (os.getuid(), element, time.asctime().replace(' ', '_'))
                    directory = "%s/manual_optical_alignment" % os.getenv("HOME")
                    if dm.oa.logger.handlers:
                        dm.oa.logger.handlers = []
                    dm.oa.move_zoom = False #True
                    dm.oa.save_history = True
                    dm.oa.directory = directory
                    dm.oa.name_pattern = name_pattern
                    dm.oa.history_start = time.time()
                    dm.oa.start_run_time = time.time()
                    dm.oa.descriptions = dm.oa._align_eagerly()
                    dm.oa.analyze()
                    dm.oa.conclude()
                    dm.oa.history_end = time.time()
                    dm.oa.end_run_time = time.time()
                    dm.oa.clean()
                    end_align = time.time()
                    log.info('Automatic sample alignment took %.2f seconds' % (end_align - start_align))
                    dm.goniometer.insert_backlight()
                    #dm.goniometer.set_frontlightlevel(12)
                else:
                    dm.start_centring_method(dm.MANUAL3CLICK_MODE)
                
                #try:
                    #mount_end = time.time()
                    #name_pattern = "mount_%s_element_%s_%s" % (os.getuid(), element, time.ctime(mount_start).replace(' ', '_'))
                    #cman.save_history(os.path.join(directory, name_pattern), mount_start, mount_end)
                #except:
                    #log.exception(traceback.format_exc)
                
                view.setText(1, "Centring !")
                
                centring_result = {'valid': True} #async_result.get()
