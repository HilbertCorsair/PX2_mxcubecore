    ...
    def collect_finished(self, owner, state, message, *args):
        ...
        try:
            ...
            if dc.experiment_type is EXPERIMENT_TYPE.MESH:
                dc.grid.set_overlay_pixmap(
                    HWR.beamline.collect.experiment.get_overlay_image_name()
                )
        except:
            logging.getLogger("HWR").exception(traceback.format_exc())
...
class XRFSpectrumQueueEntry(BaseQueueEntry):
    ...
    def xrf_spectrum_finished(self, mcaData, mcaCalib, mcaConfig):
        logging.getLogger().info(
            "xrf_spectrum_finished mcaData %s mcaCalib %s mcaConfig %s"
            % (str(mcaData[:20]), str(mcaCalib), str(mcaConfig))
        )
        xrf_spectrum = self.get_data_model()
        ...
        if dm is not None:
            ...
            try:
                ...
                if centring_method == CENTRING_METHOD.MANUAL:
                    ...
                    dm.oa.innermost_start_time = time.time()
                    dm.oa.descriptions = dm.oa._align_eagerly()
                    dm.oa.innermost_end_time = time.time()
                    ...
                    end_align = time.time()
                    log.info(
                        "Automatic sample alignment took %.2f seconds"
                ...
                # try:
                # mount_end = time.time()
                # name_pattern = "mount_%s_%s_%s" % (os.getuid(), element, time.ctime(mount_start).replace(' ', '_'))
