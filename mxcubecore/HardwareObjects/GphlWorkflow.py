...
class GphlWorkflow(HardwareObject): #, speech):
    ...
    def __init__(self, name):
        ...
        #speech.__init__(self, port=5555, service="gphl_workflow", verbose=False)
    ...
    def get_available_workflows(self):
        ...
        for wf_node in self["workflows"]:
            ...
            variants = wf_node.get_property("variants").strip().split()
            wf_dict = {
                "name": name,
    ...
    def setup_data_collection(self, payload, correlation_id):
        ...
        if bst and self.get_property("starting_beamline_energy") == "configured":
            ...
            initial_energy = HWR.beamline.energy._calculate_energy(bst.wavelength)
        ...
        wavelengths = list(
            GphlMessages.PhasingWavelength(
                wavelength=HWR.beamline.energy._calculate_wavelength(val), role=tag)
            for tag, val in beam_energies.items()
        )
        ...
        if self.get_property("disable_recen"):
    ...
    def calculate_recentring(
        ...
        envs = {"autoPROC_home": HWR.beamline.gphl_connection.software_paths.get("GPHL_INSTALLATION")}
    ...
    def collect_data(self, payload, correlation_id):
        ...
        logging.getLogger("HWR").info('GphlWorkflow collect_data collection_proposal %s' % str(collection_proposal))
        angular_tolerance = float(self.get_property("angular_tolerance", 1.0))
    ...
    def select_lattice(self, payload, correlation_id):
        ...
        display_energy_decimals = int(self.get_property("display_energy_decimals", 4))
        ...
        def resolution_is_valid(widget):
            """ Check if resolution is valid with current energy parameter

            """
            field_widget = widget.parent()
            parameters = field_widget.get_parameters_map()
            resolution = float(parameters.get("resolution", 0))
            energy = (
                float(parameters.get("energy", 0))
                or round(HWR.beamline.energy.get_value(), display_energy_decimals)
            )
            wavelength = HWR.beamline.energy._calculate_wavelength(energy)
            distance_limits = HWR.beamline.detector.distance.get_limits()
            limits = tuple(
                HWR.beamline.resolution.distance_to_resolution(dist, wavelength)
                for dist in distance_limits
            )
            return (limits[0] < resolution < limits[1])
        ...
        field_list = [
            ...
            },
            {
                ...
                "decimals": 3,
                "readOnly": False,
                "extra_validator":resolution_is_valid,
            },
        ]
        prev_energy = round(HWR.beamline.energy.get_value(), display_energy_decimals)
        energy_limits = HWR.beamline.energy.get_limits()
        field_list.append(
            {
                "variableName": "energy",
        ...
        if round(energy, display_energy_decimals) != prev_energy:
            HWR.beamline.energy.set_value(energy, timeout=60)
        wavelength = HWR.beamline.energy._calculate_wavelength(energy)
        ...
        angular_tolerance = self.get_property("angular_tolerance", 0)
        if angular_tolerance:
